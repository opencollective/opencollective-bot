...
- After a succesfull registration, generate a `private key`(file with a `.pem` extension ), rename to `private.pem` and store in the root project folder .
...
> `Dotenv-cli` is used to handle credentials within this project
...
```js
 ...
 PRIVATE_KEY_PATH = < `.pem` file placed in the `cwd`> e.g `PRIVATE_KEY_PATH = "private.pem"`
```

- **APP_ID** takes the App_id found at the **about** page.
