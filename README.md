...
> This abbreviated guide is meant to give a guide as to how github apps are made and basic steps to reproduce the OpenCollective Bot should you wish to make futher changes.
...
**WebHooks** are crucial to production **Github Apps**. Hence, you can use [Ngrok](https://ngrok.com) , [Smee](https://smee.io) or other comfortable options .

> **Smee** which is supported by **Probot** is however used in this guide .

### Setting up Smee

> Visit the [Smee](https://smee.io) website, to start a new channel

-After creating a channel, take note of the **Webhook Proxy URL** shown as it would be used as the **Webhook Url** in the about page when resgistering the app on github
...
- For the **Homepage URL**, use the **Webhook Proxy URL** generated by Smee

- After a succesfull registration, generate a `private key`(file with a `.pem` extension ) and store.
...
> Using a `.env` to handle credentials in this project is highly recommended.
...
```js
 APP_ID = <App ID>
 WebHook_Secret =
 Private_Key = <path to the private key downloaded from github>
```

- Copy the App Id found at the **about** page and insert into the App Id value in the `.env` file .

- Open the `Private key` in a code editor to view the contents and copy the contents surronding them in double quotes.
  Example `" -----BEGIN RSA PRIVATE KEY----- [content] -----END DSA PRIVATE KEY-----"`
